...
public class Task08
{
	...
	public static void main(String args[])
	{
		...
		OntClass personClass = model1.getOntClass(ns+"Person");
		ExtendedIterator personIterator = personClass.listInstances();

		while (personIterator.hasNext()) {
			Resource person = (Resource) personIterator.next();

			StmtIterator stIterator = model2.listStatements(person, null, (RDFNode) null);
			while (stIterator.hasNext())
			{
				...
				StmtIterator stIteratorModel1 = model1.listStatements(person, prop, (RDFNode) null);
				if (!stIteratorModel1.hasNext()) {
					model1.add(person, prop, fn);
				}
			}
		}
