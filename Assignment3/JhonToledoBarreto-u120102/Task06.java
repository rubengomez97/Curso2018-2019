...
public class Task06
{
	...
	public static void main(String args[])
	{
		...
		String jhonFile = "resources/eventos.rdf";
		
		// Create an empty model
		OntModel model = ModelFactory.createOntologyModel(OntModelSpec.RDFS_MEM);
		OntModel jhonModel = ModelFactory.createOntologyModel(OntModelSpec.RDFS_MEM);
		
		// Use the FileManager to find the input file
		InputStream in = FileManager.get().open(filename);
		InputStream jhonIn = FileManager.get().open(filename);
		if (in == null || jhonIn == null)
			throw new IllegalArgumentException("File: "+filename+" not found");
	
		// Read the RDF/XML file
		model.read(in, null);
		jhonModel.read(jhonIn,null);
		...
		Property work = model.createProperty(ns+"worksIn");
		...
		model.getIndividual(ns+"JohnSmith").addProperty(work, "UPM");
